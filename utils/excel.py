...
class Excel:
    ...
    def run(self, filename = 'Expenses.xlsx', dir = 'output'):
        border = 1
        workbook = xlsxwriter.Workbook(f'{dir}\\{filename}')
        ...
        write_nonbold_bold(sheet1, f'A{row + 10}:H{row + 10}', merge_format4, merge_format4_bold,
                           '九、结算方式及期限：', self.c.get_jiaohuo())
        write_nonbold_bold(sheet1, f'A{row + 11}:H{row + 11}', merge_format4, merge_format4_bold,
                           '十、如需提供担保，另立合同担保书，作为本合同附件：', self.c.get_jiaohuo())
        write_nonbold_bold(sheet1, f'A{row + 12}:H{row + 12}', merge_format4, merge_format4_bold,
                           '十一、违约责任：', self.c.get_jiaohuo())
        write_nonbold_bold(sheet1, f'A{row + 13}:H{row + 13}', merge_format4, merge_format4_bold,
                           '十二、解决合同纠纷的方式：', self.c.get_jiaohuo())
        write_nonbold_bold(sheet1, f'A{row + 14}:H{row + 14}', merge_format4, merge_format4_bold,
                           '十三、其它约定事情:', self.c.get_jiaohuo())

        for i in range(1, row + 15):
            sheet1.set_row(i, 20)
        ...
        for product_id, number, discount in self.c.get_table():
            product = self.c.get_product(product_id)
        ...
        write_nonbold_bold(sheet2, f'A{row+4}:D{row+4}', merge_format4, merge_format4_bold, '单位名称（章）：', c.get_supplier())
        write_nonbold_bold(sheet2, f'E{row+4}:K{row+4}', merge_format4, merge_format4_bold, '单位名称（章）：', c.get_buyer())
...
def write_nonbold_bold(sheet, mRange, format_nonbold, format_bold, s1, s2):
    sheet.merge_range(mRange, None)
